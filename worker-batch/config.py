...
class Settings(BaseSettings):
    ...
    # Выбрасывать строчки из титров, которые Whisper выдумывает на музыке
    # и тишине: «Субтитры сделал DimaTorzok», «Редактор субтитров …»,
    # «Продолжение следует...» целой фразой. Настоящую речь не трогает —
    # см. hallucinations.py и tests/test_hallucinations.py.
    ...
    # Языковая модель для полировки чистовика (сервис llm на Ollama).
    # Надстройка: недоступна модель — чистовик уходит без полировки.
    llm_url: str = "http://llm:11434"
    llm_model: str = "qwen3:8b"
    # Модель выгружается из видеопамяти, если столько простояла без дела.
    llm_keep_alive: str = "2m"
    # Первое обращение после простоя грузит модель — это десятки секунд.
    llm_timeout: float = 180.0
    # Поправка стыков реплик: модель выбирает, с какого слова на самом деле
    # начинается новая реплика, не дальше llm_max_shift слов от границы.
    # ВЫКЛЮЧЕНО по замеру (bench/eval_llm.py, три встречи AMI): с Qwen3-8B
    # слов под чужим именем 5,57% → 5,92–6,29% во всех вариантах, хуже на
    # каждой встрече. Модель двигает границы охотнее, чем стоит.
    llm_fix_speakers: bool = False
    llm_max_shift: int = 3
    # Название, краткое содержание и подсказка имён говорящих.
    llm_notes: bool = True
